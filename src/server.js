...
const token =
  "ILl4k1G3tJrx1dNXqkmS8nk5ty2Rlk1iRYkqz5bwpWeMOlCzyD-8!-1821777368";
...
async function obtemResultados() {
  try {
    const res = await axios.get(
      `${api}statisticHistory?tableId=spacemanyxe123nh&numberOfGames=${games}&JSESSIONID=${token}&game_mode=lobby_desktop`
    );
...
async function getApi() {
  try {
    const token =
      "FtJ1kaOEoC2z2j98dy4iJLw2gy8wIsxnqXfIAJPLR4O7D0ymcGle!-2025692314";
    const api = "https://gs9.pragmaticplaylive.net/api/ui/";
    const games = 100;
    const response = await axios.get(
      `${api}statisticHistory?tableId=spacemanyxe123nh&numberOfGames=${games}&JSESSIONID=${token}&game_mode=lobby_desktop`
    );
    const probabilidade = await axios.get(
      `${api}SMSummaryResults?tableId=spacemanyxe123nh&noOfGames=${games}&JSESSIONID=${token}&ck=1670547221537&game_mode=lobby_desktop`
    );
    const resultProb = JSON.stringify(probabilidade.data.data)
      .replace(".", ",")
      .replace("{", "")
      .replace("}", "")
      .replace("-", "até");
    ...
    if (
      response.data.history[0].gameResult < 2 &&
      response.data.history[1].gameResult < 2
    ) {
      ...
      await postTelegram(
        "🚀 ENTRADA APÓS: " + resultado.replace(".", ",") + "x \n\nATÉ G2 🚀"
      );
      console.info("ENTRAR APÓS VELA: ", resultado);
    } else if (
      response.data.history[0].gameResult > 6 &&
      response.data.history[0].gameResult < 15
    ) {
      const resultado = response.data.history[0].gameResult;
      const horario = new Date().toTimeString();
      await postTelegram(
      ...
      const msg =
        ...
        response.data.history.map(
          x => `  ${x.gameResult.toString().replace(".", ",")}  `
        );
      //await postTelegram(msg);
      console.info(
        "ULTIMOS RESULTADOS ",
        response.data.history.map(res => res.gameResult)
      );
    }
...
async function postTelegram(message) {
  try {
    const response = await axios.get(
      `https://api.telegram.org/bot5901528140:AAEz3C_OMuV50t81If4yw-rS0fpLKJy6BV4/sendMessage?chat_id=-1001611576487&text=${message}&parse_mode=MarkdownV2`
    );
